...
public class NewStripeConnectActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        isSetup = getIntent().getBooleanExtra(AppConstants.EXTRA_SETUP_STRIPE , false);
    ...
    private void selectionDone(Boolean hasStripe){
        ...
        if (isSetup) {
            heading.setText("Connect With Stripe Account");
        } else {
            if (hasStripe){
...
//                }
    ...
    private void getJsonCallback(String url) {
        new HitPayAPI(NewStripeConnectActivity.this).getJsonCallback(url, new OnComplete<JSONObject>() {
            @Override
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        if (errorMessage != null) {
                            endLoading();
                            AppManager.showErrorAlert(NewStripeConnectActivity.this, errorMessage);
                        } else {
                            submitJson(response);
                        }
                    }
    ...
    private void submitJson(final JSONObject jsonObject) {
        String bussinessID = ApplicationPreferencesManager.getStringValue(NewStripeConnectActivity.this, AppConstants.PREF_BUSSINESS_ID);
        new HitPayAPI(NewStripeConnectActivity.this).submitJson(jsonObject, bussinessID,  new OnComplete<JSONObject>() {
            @Override
            public void done(final JSONObject response, final String errorMessage) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        ...
                        if (errorMessage != null) {
                            ...
                            HitPayAPI api = new HitPayAPI(NewStripeConnectActivity.this);
        ...
        new HitPayAPI(this).getStripeUrlForHitpaySignup(bussinessID, new OnComplete<String>() {
            ...
            public void done(final String url, final String errorMessage) {
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        if (url == null){
                            AppManager.showErrorAlert(NewStripeConnectActivity.this,"Error",errorMessage);
                            return;
                        }
                        webView.loadUrl(url);
                    }
